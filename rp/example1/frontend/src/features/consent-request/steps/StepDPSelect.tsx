...
export function StepDPSelect() {
  ...
  function formatThaiDate(iso: string): string {
    ...
    return `${day}/${month}/${String(thaiYear).slice(-2)}`
  }
      ...
      <div className="flex-1 p-4 overflow-y-auto">
        ...
        >
          <svg width="11" height="11" viewBox="0 0 10 10" fill="none" aria-hidden="true">
            <rect x="1" y="2" width="8" height="7" rx="1" stroke="#0369A1" strokeWidth="1.2" />
            <path d="M1 4.5h8M3.5 2V1.5M6.5 2V1.5" stroke="#0369A1" strokeWidth="1.2" strokeLinecap="round" />
          </svg>
        ...
        <p className="text-xs mb-3" style={{ color: '#64748B' }}>
          กรุณาเลือกธนาคารที่ประสงค์ให้ส่งข้อมูลเงินฝาก
        </p>
        ...
        >
          <div className="flex items-center justify-between mb-1.5">
            ...
            <span className="text-sm font-bold" style={{ color: selectedCount >= MAX_DP_COUNT ? '#DC2626' : '#0F172A' }}>
              {selectedCount} / {MAX_DP_COUNT} ราย
            </span>
          </div>
          <div className="h-1.5 rounded-full overflow-hidden" style={{ background: '#E2E8F0' }}>
            <div
              ...
              style={{
                width: `${progressPct}%`,
                background: selectedCount >= MAX_DP_COUNT ? '#DC2626' : '#0F172A',
              }}
            />
          </div>
        </div>
        ...
        >
          ...
          <p style={{ fontSize: 12.5, color: '#0F172A', marginTop: 3 }}>
            ย้อนหลัง {months} เดือน นับจากวันที่เรียก
          </p>
