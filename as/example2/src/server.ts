/**
 * AS example2 — Beta Bank (Deposit-Only Provider)
 *
 * This AS represents a second bank that offers deposit accounts only.
 * It handles:
 *   NDID services    : pre_consent_deposit, revoke_consent
 *   Your Data services: complete_consent, deposit_transactions
 *
 * Differences from example1 (Alpha Bank):
 *   - Deposit-only (no credit card)
 *   - Different account data (Beta Bank accounts)
 *   - Ports: callback=6003, server=11000, NDID API=8400
 */
...
// Import callback handler to start the callback server
...
async function registerServices(): Promise<void> {
  // Standard NDID services handled by this AS (deposit only)
  // Service IDs per NDID_YourData_ErrorCodes-ServiceID-ServiceExtension sheet "3_Service ID"
  const ndidServices = ['900.pre_consent_deposit_001', '900.revoke_consent_001'];
...
// account number the consent token itself must carry. The masked
...
/** Mask a real account number down to its last 4 digits, e.g.
 * "987-654-9001" -> "***-***-9001". Separators are preserved as-is. */
...
ndidCallbackEvent.on('ndid_callback', (data: NdidAsCallback) => {
  if (data.type === 'data_request') {
    ...
    if (result.success) {
      console.log(`NDID data sent successfully for request: ${result.request_id}`);
    } else {
    ...
    console.log(`NDID request ${statusData.request_id} status: ${statusData.status}`);
  }
});

async function handleNdidDataRequest(data: NdidDataRequestCallback): Promise<void> {
  const { request_id, service_id } = data;
  const referenceId = `ndid-data-${Date.now()}`;

  console.log(`Handling NDID data request for service: ${service_id}, request: ${request_id}`);
  ...
  try {
    if (service_id === '900.revoke_consent_001') {
    ...
    } else if (service_id === '900.pre_consent_deposit_001') {
      ...
      // identifier is an opaque unique ID generated by the AS — NOT the real
      // account number. visible_identifier is a masked view of the same real
      // number in ACCOUNT_REAL_NUMBER, so the two are never inconsistent.
      intent.sub_identity_list = [
        {
          namespace: 'account_id',
          identifier: 'beta-dep-c1d2e3f4',
          visible_identifier: maskAccountNumber(ACCOUNT_REAL_NUMBER['beta-dep-c1d2e3f4']),
          identifier_extension: '{"accountSubType":"CURRENT"}',
        },
        {
          namespace: 'account_id',
          identifier: 'beta-dep-g5h6i7j8',
          visible_identifier: maskAccountNumber(ACCOUNT_REAL_NUMBER['beta-dep-g5h6i7j8']),
      ...
      // Store intent for lookup at complete-consent.
...
async function handleYourDataRequest(data: YourDataAsDataRequestCallback): Promise<void> {
  const { request_id, service_id } = data;

  console.log(`Handling Your Data request for service: ${service_id}, request: ${request_id}`);
  ...
  try {
    if (service_id === '900.complete_consent_001') {
      ...
      try {
        requestParams = JSON.parse(data.request_params || '{}');
      } catch {
        // Malformed JSON — ignore, treat as empty params
      }
      ...
      await API.sendYourData({
        request_id,
        data: JSON.stringify(getDepositTransactions(extension, tokenAccount, dateRange)),
      });
      // Mark one-time token as consumed after data is successfully sent.
      markTokenUsedIfOneTime(request_id, data.authorization);
    } else {
      await API.sendYourDataError({
        request_id,
        error_code: 40740,
        error_message: `Unsupported service: ${service_id}`,
      });
    }
  } catch (error) {
...
// ACCOUNT_REAL_NUMBER at lookup time (see getDepositTransactions) so it can
// never drift from the number used everywhere else for the same account.
...
function getDepositTransactions(
  ...
  const mock = DEPOSIT_MOCK[account.identifier] ?? DEPOSIT_MOCK['beta-dep-c1d2e3f4'];
  ...
  // same real number carried in the consent token.
...
app.get('/health', (_req: Request, res: Response) => {
  res.status(200).json({ status: 'ok', service: 'as-example2-beta-bank-deposit' });
});
...
app.listen(config.serverPort, () => {
  console.log(`AS example2 (Beta Bank deposit) server running on port ${config.serverPort}`);
});
